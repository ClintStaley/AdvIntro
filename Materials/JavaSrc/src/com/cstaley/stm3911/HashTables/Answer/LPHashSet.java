...
public class LPHashSet<E> implements Set<E> {
   ...
   private void rehash(int incr) {
      Object[] oldData = table;
      table = new Object[table.length + incr];
      
      valCount = 0;
      steps = 0;
      nullCount = table.length;
      for (Object entry: oldData)
         if (entry != null && entry != wasFull)
            add((E)entry);
   }
   ...
   public String toString() {
      ...
      bld.append("Avg steps: ").append(valCount == 0 ? "N/A" : (float)steps/valCount);
      return bld.toString();      
   }
