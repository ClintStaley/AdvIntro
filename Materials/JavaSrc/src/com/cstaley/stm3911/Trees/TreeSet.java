...
import com.cstaley.stm3911.Stacks3.ans.LinkStack;
...
public class TreeSet<E> implements Set<E> {
   ...
   public boolean contains(Object toFind) {
      ...
      while (nd != null && (res = cmp.compare((E)toFind, (E)nd.data)) != 0) 
   ...
   public boolean add(E toAdd) {
      ...
      // Walk down the tree till we find toFind or fall off
      // Use only one cmp.compare call per iteration
      while (child != null && (res = cmp.compare(toAdd, (E)child.data)) != 0) {
         parent = child;
   ...
   public boolean remove(Object toDrop) {
      ...
      while (child != null 
         && (res = cmp.compare((E)toDrop, (E)child.data)) != 0) {
         parent = child;
         child = res < 0 ? child.left : child.right;
      }
   ...
   private class TSItr implements Iterator<E> {
      private LinkStack<Node> path; // Path of nodes not yet returned by next.
   ...
   // Add an indented diagram of "subRoot" to "bld", with one line per node. 
   // Assume the entire diagram is within a larger tree and is indented by
   // "level" columns within that tree.
