...
public class DFSSolver implements Solver {
   ...
   private static class DFSSolution extends Solution {
      // Construct the solution by copying the steps from the stack
      public DFSSolution(ArrStack<DFSStep> stack) {
         mCost = stack.getSize() - 1;
         mSteps = new Problem.Step[stack.getSize()];
         for (int ndx = mSteps.length - 1; ndx >= 0; ndx--) {
            mSteps[ndx] = stack.top().mStep;
            stack.pop();
         }
      }
   }
   ...
   public Solution[] solveProblem(Problem prb, int maxCost, int numSlns) {
      ...
      while (!stack.isEmpty() && slns.size() < numSlns) {
         ...
         if (top.mItr.hasNext() && stack.getSize() - 1 < maxCost) {
            step = top.mItr.next();
            ...
            if (!stack.isOn(newStep)) {
               ...
               if (newState.isGoal()) {  // If solution
                  ...
                  slns.add(new DFSSolution(cpyStack));
                  stack.pop();
               }
            }
         }
